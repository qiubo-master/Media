...
export default async function Home() {
  ...
  const [[audience], [performance], trendRows, accountRows, topRows, dailyAccountRows] = await Promise.all([
    ...
    db.execute(sql`select d.day::date as date,a.id as account_id,coalesce(a.display_name,a.handle) as account_name,a.platform,coalesce(sum(m.views),0)::int as views,coalesce(sum(m.leads),0)::int as leads,coalesce(sum(m.revenue),0)::numeric as revenue from generate_series(current_date - interval '29 days',current_date,interval '1 day') d(day) cross join accounts a left join contents c on c.account_id=a.id left join metrics m on m.content_id=c.id and m.metric_date=d.day::date where a.ip_id=${profile.id} group by d.day,a.id order by d.day,a.created_at`),
  ]);
  ...
  const dailyAccounts = Array.from(dailyAccountRows).map((row) => ({ date: String(row.date), accountId: String(row.account_id), accountName: String(row.account_name), platform: String(row.platform), views: Number(row.views), leads: Number(row.leads), revenue: Number(row.revenue) }));
  return <DashboardClient userName={user.name} ipName={profile.name} isAdmin={user.role === "admin"} stats={{ followers: Number(audience?.followers || 0), views: Number(performance?.views || 0), leads: Number(performance?.leads || 0), revenue: Number(performance?.revenue || 0) }} trends={trends} accountTrends={accountTrends} topContents={topContents} dailyAccounts={dailyAccounts} />;
}

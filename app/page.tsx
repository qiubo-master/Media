  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom"><button onClick={() => setPanel(true)}>⚙　模型与系统设置</button><div className="usage"><span>本月 AI 用量</span><b>68%</b><i><u /></i><small>68.2万 / 100万 Tokens</small></div></div>
      </aside>

      <section className="content">
        <header className="topbar"><div><span className="status-dot" />所有账号运行正常</div><div className="top-actions"><button aria-label="搜索">⌕</button><button aria-label="通知">♢<i /></button><button onClick={() => setPanel(true)} className="model-pill"><span>✦</span>{providers.find(p => p.id === provider)?.name} · {model}<b>⌄</b></button><span className="user-avatar">林</span></div></header>

        <div className="page">
          <div className="welcome"><div><p>2026年8月10日 · 星期一</p><h1>早上好，林野 <span>👋</span></h1><h2>这是你的内容生意全景，今天有 <b>3 件事</b>值得优先处理。</h2></div><div className="range">{["近7天", "近30天", "本季度"].map(r => <button onClick={() => setRange(r)} className={range === r ? "active" : ""} key={r}>{r}</button>)}</div></div>

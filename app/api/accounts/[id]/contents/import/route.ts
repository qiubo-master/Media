...
import { normalizeSpreadsheetDate, readUploadedTable } from "@/lib/tabular-upload";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  let table;
  try { table = await readUploadedTable(file); } catch { return seeOther(`/accounts/${id}?error=file_type`); }
  const header = table.headers.map((key) => aliases[key] || key.toLowerCase());
  if (!header.includes("title") || !header.includes("metric_date")) return seeOther(`/accounts/${id}?error=columns`);
  if (!table.rows.length) return seeOther(`/accounts/${id}?error=empty_file`);
  const [batch] = await db.insert(importBatches).values({ ownerId: user.id, accountId: id, fileName: file.name, importType: "content_metrics", rowCount: table.rows.length }).returning({ id: importBatches.id });
  const errors: { row: number; message: string }[] = []; let success = 0; let followerDifference = 0;
  for (let index = 0; index < table.rows.length; index++) {
    try {
      const values = table.rows[index]; const row = Object.fromEntries(header.map((key, column) => [key, values[column] || ""]));
      const metricDate = normalizeSpreadsheetDate(row.metric_date); const publishedDate = normalizeSpreadsheetDate(row.published_date || "");
      if (!row.title || !metricDate) throw new Error("缺少标题或数据日期无效");
      ...
      if (!work) [work] = await db.insert(contents).values({ ipId: owned.ipId, accountId: id, title: row.title, status: "published", format: row.format || "短视频", publishedUrl: row.url || null, publishedAt: publishedDate ? new Date(`${publishedDate}T12:00:00+08:00`) : null }).returning({ id: contents.id });
      ...
      const [previous] = await db.select({ followerDelta: metrics.followerDelta }).from(metrics).where(and(eq(metrics.contentId, work.id), eq(metrics.metricDate, metricDate))).limit(1);
      ...
      await db.insert(metrics).values({ contentId: work.id, metricDate, ...metricValues, engagements: metricValues.likes + metricValues.saves + metricValues.shares }).onConflictDoUpdate({ target: [metrics.contentId, metrics.metricDate], set: { ...metricValues, engagements: metricValues.likes + metricValues.saves + metricValues.shares, capturedAt: new Date() } });
      followerDifference += followerDelta - (previous?.followerDelta || 0); success++;
    } catch (error) { errors.push({ row: index + 2, message: error instanceof Error ? error.message : "解析失败" }); }
  }

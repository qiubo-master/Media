...
export default async function Login({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const demoPreview = process.env.LOCAL_DEMO_MODE === "true" && query.preview === "1";
  if (!demoPreview && await getCurrentUser()) redirect("/");
  const [result] = demoPreview ? [{ value: 1 }] : await db.select({ value: count() }).from(users);

...
export default async function AccountsPage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return <main className="module-page">
    ...
    <section className="module-card full"><div className="section-title"><div><h2>账号列表</h2><p>最近一条上传记录会显示在这里。</p></div></div>
      {!rows.length ? <div className="empty">还没有账号，请先添加第一个平台账号。</div> : <div className="account-table"><div className="account-row head"><span>账号</span><span>同步方式</span><span>粉丝</span><span>播放</span><span>互动</span><span>线索</span><span>数据日期</span></div>{rows.map((account) => { const metric = latestByAccount.get(account.id); return <div className="account-row" key={account.id}><span><b>{account.displayName || account.handle}</b><small>{account.platform}</small></span><span><i className={account.syncMode === "api" ? "api" : "manual"}>{account.syncMode === "api" ? "API 自动" : "手工上传"}</i></span><span>{metric?.followers?.toLocaleString() ?? account.followers.toLocaleString()}</span><span>{metric?.views?.toLocaleString() ?? "—"}</span><span>{metric?.engagements?.toLocaleString() ?? "—"}</span><span>{metric?.leads?.toLocaleString() ?? "—"}</span><span>{metric?.date ?? "尚未上传"}</span></div>})}</div>}
    </section>
